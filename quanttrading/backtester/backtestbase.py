...
class BacktestingEngine:
    ...
    def cross_limit_order(self) -> None:
        """
        stimulate trade on limit order with last bar/tick data by on_trade
        and change the order status by on_order
        simulate the market to check if the order could be filled.
        if order could be filled. 
        """
        if self.mode == BacktestingMode.BAR:
